...
const size_t MAX_FEATURES = 50000;
...
float process(CMS<N>& sketch, tk_t& topk, const x_t& x, bool train, bool neglabel)
// modded RS
{
// modded RS
...
void consumer(CMS<N>& sketch, tk_t& topk, fast_parser& p, mp_queue<x_t>& q, bool train, bool neglabel)
// modded RS
{
...
//						loss += process(sketch, topk, items[cdx], train);
						loss += process(sketch, topk, items[cdx], train, neglabel);
// modded RS
				}
...
{
// modded RS
        const size_t D = atoi(argv[3]);
        bool neglabel = atoi(argv[4]);
// modded RS
		CMS<N> sketch(K, D);
...
//		std::thread train_cr([&] { consumer(sketch, topk, train_p, q, true); });
		std::thread train_cr([&] { consumer(sketch, topk, train_p, q, true, neglabel); });
// modded RS
		train_pr.join();
		train_cr.join();
...
//		std::thread test_cr([&] { consumer(sketch, topk, test_p, q, false); });
		std::thread test_cr([&] { consumer(sketch, topk, test_p, q, false, neglabel); });
// modded RS
		test_pr.join();
		test_cr.join();
